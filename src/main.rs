use ashira_tokenizer_v2::{scan_training_files, TokenizerTrainer, TrainConfig, VOCAB_SIZE};
use std::env;
...
fn main() {
    ...
    println!("=== AshiraTokenizer v2 Trainer ===");
    ...
    println!("[SCAN] files={} skipped={} bytes={}", summary.total_files, summary.skipped_files, summary.total_bytes);
    ...
    if let Err(e) = fs::write(&config_path, config_json.as_bytes()) {
        ...
        let _ = writeln!(log, "[{}] [ERROR] config write failed: {}", now_utc_epoch(), e);
        std::process::exit(7);
    }

    println!("[DONE] vocab={} merges={} duration={}s", stats.final_vocab, stats.learned_merges, stats.duration_seconds);
